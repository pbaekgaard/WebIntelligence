def aggregate(vsm_results, page_rank_content, beta=0.20):
    # Create a dictionary for quick lookup of PageRank scores by URL
    page_rank_dict = dict(page_rank_content)

    # Prepare the aggregated scores as a list of tuples
    aggregated_scores = []
    ...
    for url, vsm_score in vsm_results:
        ...
        aggregated_score = (beta * vsm_score) + ((1 - beta) * (page_rank_score*10))

        # Append the result as a tuple
        aggregated_scores.append((url, aggregated_score))

    # Sort scores in descending order based on the aggregated score
